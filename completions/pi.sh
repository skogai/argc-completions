...
# @option --thinking[off|minimal|low|medium|high|xhigh|max] <level>  Set thinking level: off, minimal, low, medium, high, xhigh, max
...
# @option --use-theme <name[/name]>        Set the initial interactive theme for this run
...
# @option --tui-mode <mode>                TUI mode: regular (default) or fullscreen
...
# {{ pi update
# @cmd Update pi, extensions, or model catalogs
# @flag --self                    Update pi only (default when no target is given)
# @flag --extensions              Update installed packages only
# @flag --models                  Refresh model catalogs only
# @flag --all                     Update pi and installed packages
...
# {{ pi config
# @cmd Open TUI to enable/disable package resources (Tab switches scope)
# @flag -l --local      Edit project overrides (.pi/settings.json)
# @flag -a --approve    Trust project-local files for this command with -l
# @flag -na             Ignore project-local files for this command with -l
# @flag --no-approve    Ignore project-local files for this command with -l
config() {
    :;
}
# }} pi config
