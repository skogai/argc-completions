...
# @option -J <destination>                      Connect to the target host by first making an ssh connection to the jump host described by destination and then establishing a TCP forwarding to the ultimate destination from there.
...
# @option -P <tag>                              Specify a tag name that may be used to select configuration in ssh_config(5).
