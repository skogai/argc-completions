...
# @flag -s                            If the -s option is present, or if no arguments remain after option processing, the shell reads commands from the standard input.
# @flag -D                            Print a list of all double-quoted strings preceded by $ on the standard output.
...
# @option --rcfile <file>             Execute commands from file instead of the standard personal initialization file ~/.bashrc if the shell is interactive (see INVOCATION below).
...
# @flag --norc                        Do not read and execute the personal initialization file ~/.bashrc if the shell is interactive.
# @flag --posix                       Enable posix mode; change the behavior of bash where the default operation differs from the POSIX standard to match the standard.
