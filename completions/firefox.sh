...
# @option --origin-to-force-quic-on <origin>    Force to use QUIC for the specified origin.
...
# @option --remote-debugging-port <port>        Start the Firefox Remote Agent, which is a low-level remote debugging interface used for WebDriver BiDi.
...
# @flag --remote-allow-system-access            Enable privileged access to the application's parent process
# @arg paths*
