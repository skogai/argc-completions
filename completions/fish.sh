...
# @option -c --command                        Evaluate the specified commands instead of reading from the commandline.
...
# @option -o --debug-output <DEBUG_FILE>      Specifies a file path to receive the debug output, including categories and fish_trace <#envvar-fish_trace>.
...
# @flag -P --private                          Enables private mode <#private-mode>: fish will not access old or store new history.
...
# @option -f --features                       Enables one or more comma-separated feature flags <#featureflags>.
# @arg file
# @arg args*
