...
# @option --popup[center|top|bottom|left|right] <OPTS>  Start fzf in a floating pane (requires tmux 3.3+ or Zellij 0.44+)[,SIZE[%]][,SIZE[%]] [,border-native] (default: center,50%)
...
# @option --preview-window[up|down|left|right|next] <OPT>  Preview window layout (default: right:50%)[,SIZE[%]] [,[no]wrap[-word]][,[no]cycle][,[no]follow][,[no]info] [,[no]hidden][,border-STYLE] [,+SCROLL[OFFSETS][/DENOM]][,~HEADER_LINES] [,default][,<SIZE_THRESHOLD(ALTERNATIVE_LAYOUT)]
...
# @option --listen <[ADDR:]PORT|SOCKET_PATH>     Start HTTP server to receive actions via TCP (To allow remote process execution, use --listen-unsafe)
...
# @flag --nushell                                Print script to set up Nushell integration
