...
# @flag --file-type                            like -F, except do not append '*'
# @option --format[`_choice_format`] <WORD>    across,horizontal (-x), commas (-m), long (-l), single-column (-1), verbose (-l), vertical (-C)
...
# @option --hyperlink[`_choice_hyperlink`] <WHEN>  hyperlink file names WHEN
# @option --indicator-style[`_choice_indicator_style`] <WORD>  append indicator with style WORD to entry names: none (default), slash (-p), file-type (--file-type), classify (-F)
...
# @flag -p                                     append / indicator to directories
...
# @option --sort[`_choice_sort`] <WORD>        change default 'name' sort to WORD: none (-U), size (-S), time (-t), version (-v), extension (-X), name, width
...
# @flag -U                                     do not sort directory entries
...
_choice_color() {
    printf "%s\n" auto never always tty if-tty yes no force none
}
