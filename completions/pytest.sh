...
# @flag --strict-config                          Enables the strict_config option
# @flag --strict-markers                         Enables the strict_markers option
# @flag --strict                                 Enables the strict option
...
# @flag --sw-reset                               Resets stepwise state, restarting the stepwise workflow.
# @flag --stepwise-reset                         Resets stepwise state, restarting the stepwise workflow.
...
# @flag --force-short-summary                    Force condensed summary output regardless of verbosity level.
# @flag -q --quiet                               Decrease verbosity
# @option --verbosity <VERBOSE>                  Set verbosity.
# @option -r --report-chars <chars>              Show extra test summary info as specified by chars: (f)ailed, (E)rror, (s)kipped, (x)failed, (X)passed, (p)assed, (P)assed with output, (a)ll except passed (p/P), or (A)ll.
...
# @option --junitxml <path>                      Create junit-xml style report file at given path
# @option --junit-xml <path>                     Create junit-xml style report file at given path
# @option --junitprefix <str>                    Prepend prefix to classnames in junit-xml output
...
# @option --max-warnings <num>                   Exit with error if all tests pass but the number of warnings exceeds this threshold
...
# @flag --disable-plugin-autoload                Disable plugin auto-loading through entry point packaging metadata.
...
# @option -o --override-ini <OVERRIDE_INI>       Override configuration option with "option=value" style, e.g. `-o strict_xfail=True -o cache_dir=cache`.
