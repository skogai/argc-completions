...
# @flag -Q --no-queues             Suppress sending and receiving queue columns.
...
# @flag -B --bound-inactive        Display only TCP bound but inactive (not listening, connecting, etc.) sockets (these are omitted by default).
...
# @flag -p --processes             Show processes using sockets.
# @flag -T --threads               Show threads using sockets.
...
# @flag -b --bpf                   Show socket classic BPF filters (only administrators are allowed to get this information).
...
# @flag --bpf-maps                 Pretty-print all the BPF socket-local data entries for each socket.
# @option --bpf-map-id <MAP_ID>    Pretty-print the BPF socket-local data entries for the requested map ID.
# @arg filter
