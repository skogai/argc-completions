...
# @flag --setmonitor                               name geometry output-list Define a new monitor with the given geometry and associated to the given outputs.
...
# @flag --setprovideroutputsource                  provider source Set source as the source of display output images for provider.
# @flag --setprovideroffloadsink                   provider sink Set provider as a render offload device for sink.
...
# @flag --newmode                                  name mode New modelines can be added to the server and then associated with outputs.
# @option --rmmode <name>                          This removes a mode from the server if it is otherwise unused.
# @flag --addmode                                  output name Add a mode to the set of valid modes for an output.
# @flag --delmode                                  output name Remove a mode from the set of valid modes for an output.
# @option --output <output>                        Selects an output to reconfigure.
# @flag --auto                                     For connected but disabled outputs, this will enable them using their first preferred mode (or something close to 96 dpi if they have no preferred mode).
...
# @flag --set                                      property value Sets an output property.
