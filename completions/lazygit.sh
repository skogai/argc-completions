...
# @flag -p                              --path  Path of git repo.
# @flag -f                              --filter  Path to filter on in `git log -- <path>`.
# @flag -v                              --version  Print the current version
# @flag -d                              --debug  Run in debug mode with logging (see --logs flag below).
# @flag -l                              --logs  Tail lazygit logs (intended to be used when `lazygit --debug` is called in a separate terminal tab) --profile  Start the profiler and serve it on http port 6060.
# @flag -c                              --config  Print the default config
# @flag -cd                             --print-config-dir  Print the config directory
...
# @flag -w                              --work-tree  equivalent of the --work-tree git argument
# @flag -g                              --git-dir  equivalent of the --git-dir git argument
...
# @flag -sm                             --screen-mode  The initial screen-mode, which determines the size of the focused panel.
# @arg git-arg
